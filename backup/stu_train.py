...
from model import bert_classficator, student_bert_classfier,dim_converter
...
def train(teacher_model, student_model, train_loader, dev_loader, device, config):
    ...
    loss_mse = nn.MSELoss()
    converter = dim_converter(config)
    converter.to(device)
    ...
    if config.stu_hidden_size != config.tea_hidden_size:
        optimizer_grouped_parameters.append({'params': converter.parameters(), 'weight_decay': 0.0})
        flag = True
        converter.train()
    else:
        flag = False
    ...
    for epoch in range(total_epochs):
        for step, (input_ids, token_type_ids, attention_mask, labels, ids) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            # model
            tea_res, tea_hdn = teacher_model(input_ids, token_type_ids, attention_mask)
            stu_res, stu_hdn = student_model(input_ids, token_type_ids, attention_mask)

            tea_hidden_state = tea_hdn[2]
            stu_hidden_state = stu_hdn[2]
            tea_attention = tea_hdn[3]
            stu_attention = stu_hdn[3]
            ...
            if flag:
                new_student_reps = [converter(x) for x in stu_hidden_state]
            else:
